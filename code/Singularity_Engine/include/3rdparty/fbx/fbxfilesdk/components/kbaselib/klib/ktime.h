...
public:
    ...
    * \param pTimeMode Time mode identifier.
  	* \param pFrameRate in case of pTimeMode = custom, we specify the custom framerate to use: EX:12.5
  ...
  * \param pFramerate indicate custom framerate in case of pTimeMode = eCUSTOM
    ...
    */
    ...
    /** Equality operator.
	  * \param pTime KTime compared with this one.
    ...
    /** Assignment operator.
	  * \param pTime KTime assigned to this one.
	  */
    inline KTime& operator=(const KTime& pTime) {mTime = pTime.mTime; return *this;}
	
    /** Addition operator.
	  * \param pTime KTime added to this one.
	  * \return This KTime after addition.
	  */
    inline KTime& operator+=(const KTime& pTime) {mTime += pTime.mTime; return *this;}
	
    /** Subtraction operator.
	  * \param pTime KTime subtracted from this one.
