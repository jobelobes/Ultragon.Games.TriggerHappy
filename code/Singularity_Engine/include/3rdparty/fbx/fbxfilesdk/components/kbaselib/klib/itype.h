...
#ifndef FBXFILESDK_COMPONENTS_KBASELIB_KLIB_ITYPE_H
#define FBXFILESDK_COMPONENTS_KBASELIB_KLIB_ITYPE_H
...
// pointer increment
template<class T> K_INLINE T *kInc(T *p, kOffset i)
{
	return (T *) (((kInt8 *) p) + i);
}

// pointer decrement
template<class T> K_INLINE T *kDec(T *p, kOffset i)
{
	return (T *) (((kInt8 *) p) - i);
}

// pointer difference
template<class T> K_INLINE kOffset kDiff(T *p, T *q)
{
	return kOffset(p) - kOffset(q);
}
...
// conditional value
template<class T> K_INLINE T kCond(int c, T t, T f)
{
	return c ? t : f;
}
...
K_TEMPLATE_SPEC(kSChar) K_INLINE kSChar kMin(kSChar x, kSChar y)
{
	kSChar m = kSChar(x > y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kUChar) K_INLINE kUChar kMin(kUChar x, kUChar y)
{
	kUChar m = kUChar(x > y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kShort) K_INLINE kShort kMin(kShort x, kShort y)
{
	kShort m = kShort(x > y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kUShort) K_INLINE kUShort kMin(kUShort x, kUShort y)
{
	kUShort m = kUShort(x > y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(int) K_INLINE int kMin(int x, int y)
{
	int m = int(x > y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kUInt) K_INLINE kUInt kMin(kUInt x, kUInt y)
{
	kUInt m = kUInt(x > y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kLongLong) K_INLINE kLongLong kMin(kLongLong x, kLongLong y)
{
	kLongLong m = kLongLong(x > y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kULongLong) K_INLINE kULongLong kMin(kULongLong x, kULongLong y)
{
	kULongLong m = kULongLong(x > y) - 1; return (m & x) | (~m & y);
}


K_TEMPLATE_SPEC(kSChar) K_INLINE kSChar kMax(kSChar x, kSChar y)
{
	kSChar m = kSChar(x < y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kUChar) K_INLINE kUChar kMax(kUChar x, kUChar y)
{
	kUChar m = kUChar(x < y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kShort) K_INLINE kShort kMax(kShort x, kShort y)
{
	kShort m = kShort(x < y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kUShort) K_INLINE kUShort kMax(kUShort x, kUShort y)
{
	kUShort m = kUShort(x < y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(int) K_INLINE int kMax(int x, int y)
{
	int m = int(x < y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kUInt) K_INLINE kUInt kMax(kUInt x, kUInt y)
{
	kUInt m = kUInt(x < y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kLongLong) K_INLINE kLongLong kMax(kLongLong x, kLongLong y)
{
	kLongLong m = kLongLong(x < y) - 1; return (m & x) | (~m & y);
}

K_TEMPLATE_SPEC(kULongLong) K_INLINE kULongLong kMax(kULongLong x, kULongLong y)
{
	kULongLong m = kULongLong(x < y) - 1; return (m & x) | (~m & y);
}

// minimum and maximum value
K_TEMPLATE_SPEC(kSChar) K_INLINE kSChar kMinMax(kSChar x, kSChar y, kSChar *z)
{
	kSChar m = kSChar(x > y) - 1; return (*z = (x & ~m) | (y & m)), (x & m) | (y & ~m);
}

K_TEMPLATE_SPEC(kUChar) K_INLINE kUChar kMinMax(kUChar x, kUChar y, kUChar *z)
{
	kUChar m = kUChar(x > y) - 1; return (*z = (x & ~m) | (y & m)), (x & m) | (y & ~m);
}

K_TEMPLATE_SPEC(kShort) K_INLINE kShort kMinMax(kShort x, kShort y, kShort *z)
{
	kShort m = kShort(x > y) - 1; return (*z = (x & ~m) | (y & m)), (x & m) | (y & ~m);
}

K_TEMPLATE_SPEC(kUShort) K_INLINE kUShort kMinMax(kUShort x, kUShort y, kUShort *z)
{
	kUShort m = kUShort(x > y) - 1; return (*z = (x & ~m) | (y & m)), (x & m) | (y & ~m);
}

K_TEMPLATE_SPEC(int) K_INLINE int kMinMax(int x, int y, int *z)
{
	int m = int(x > y) - 1; return (*z = (x & ~m) | (y & m)), (x & m) | (y & ~m);
}

K_TEMPLATE_SPEC(kUInt) K_INLINE kUInt kMinMax(kUInt x, kUInt y, kUInt *z)
{
	kUInt m = kUInt(x > y) - 1; return (*z = (x & ~m) | (y & m)), (x & m) | (y & ~m);
}

K_TEMPLATE_SPEC(kLongLong) K_INLINE kLongLong kMinMax(kLongLong x, kLongLong y, kLongLong *z)
{
	kLongLong m = kLongLong(x > y) - 1; return (*z = (x & ~m) | (y & m)), (x & m) | (y & ~m);
}

K_TEMPLATE_SPEC(kULongLong) K_INLINE kULongLong kMinMax(kULongLong x, kULongLong y, kULongLong *z)
{
	kULongLong m = kULongLong(x > y) - 1; return (*z = (x & ~m) | (y & m)), (x & m) | (y & ~m);
}

// clamp
K_TEMPLATE_SPEC(kSChar) K_INLINE kSChar kClamp(kSChar x, kSChar y, kSChar z)
{
	kSChar m = kSChar(x < y) - 1; m = kSChar((x = (m & x) | (~m & y)) > z) - 1; return (m & x) | (~m & z);
}

K_TEMPLATE_SPEC(kUChar) K_INLINE kUChar kClamp(kUChar x, kUChar y, kUChar z)
{
	kUChar m = kUChar(x < y) - 1; m = kUChar((x = (m & x) | (~m & y)) > z) - 1; return (m & x) | (~m & z);
}

K_TEMPLATE_SPEC(kShort) K_INLINE kShort kClamp(kShort x, kShort y, kShort z)
{
	kShort m = kShort(x < y) - 1; m = kShort((x = (m & x) | (~m & y)) > z) - 1; return (m & x) | (~m & z);
}

K_TEMPLATE_SPEC(kUShort) K_INLINE kUShort kClamp(kUShort x, kUShort y, kUShort z)
{
	kUShort m = kUShort(x < y) - 1; m = kUShort((x = (m & x) | (~m & y)) > z) - 1; return (m & x) | (~m & z);
}

K_TEMPLATE_SPEC(int) K_INLINE int kClamp(int x, int y, int z)
{
	int m = int(x < y) - 1; m = int((x = (m & x) | (~m & y)) > z) - 1; return (m & x) | (~m & z);
}

K_TEMPLATE_SPEC(kUInt) K_INLINE kUInt kClamp(kUInt x, kUInt y, kUInt z)
{
	kUInt m = kUInt(x < y) - 1; m = kUInt((x = (m & x) | (~m & y)) > z) - 1; return (m & x) | (~m & z);
}

K_TEMPLATE_SPEC(kLongLong) K_INLINE kLongLong kClamp(kLongLong x, kLongLong y, kLongLong z)
{
	kLongLong m = kLongLong(x < y) - 1; m = kLongLong((x = (m & x) | (~m & y)) > z) - 1; return (m & x) | (~m & z);
}

K_TEMPLATE_SPEC(kULongLong) K_INLINE kULongLong kClamp(kULongLong x, kULongLong y, kULongLong z)
{
	kULongLong m = kULongLong(x < y) - 1; m = kULongLong((x = (m & x) | (~m & y)) > z) - 1; return (m & x) | (~m & z);
}

K_TEMPLATE_SPEC(kSChar) K_INLINE bool kIsIncl(kSChar x, kSChar y, kSChar z)
{
	return (x >= y) & (x <= z);
}

K_TEMPLATE_SPEC(kUChar) K_INLINE bool kIsIncl(kUChar x, kUChar y, kUChar z)
{
	return (x >= y) & (x <= z);
}

K_TEMPLATE_SPEC(kShort) K_INLINE bool kIsIncl(kShort x, kShort y, kShort z)
{
	return (x >= y) & (x <= z);
}

K_TEMPLATE_SPEC(kUShort) K_INLINE bool kIsIncl(kUShort x, kUShort y, kUShort z)
{
	return (x >= y) & (x <= z);
}

K_TEMPLATE_SPEC(int) K_INLINE bool kIsIncl(int x, int y, int z)
{
	return (x >= y) & (x <= z);
}

K_TEMPLATE_SPEC(kUInt) K_INLINE bool kIsIncl(kUInt x, kUInt y, kUInt z)
{
	return (x >= y) & (x <= z);
}

K_TEMPLATE_SPEC(kSChar) K_INLINE bool kIsExcl(kSChar x, kSChar y, kSChar z)
{
	return (x < y) & (x > z);
}

K_TEMPLATE_SPEC(kUChar) K_INLINE bool kIsExcl(kUChar x, kUChar y, kUChar z)
{
	return (x < y) & (x > z);
}

K_TEMPLATE_SPEC(kShort) K_INLINE bool kIsExcl(kShort x, kShort y, kShort z)
{
	return (x < y) & (x > z);
}

K_TEMPLATE_SPEC(kUShort) K_INLINE bool kIsExcl(kUShort x, kUShort y, kUShort z)
{
	return (x < y) & (x > z);
}

K_TEMPLATE_SPEC(int) K_INLINE bool kIsExcl(int x, int y, int z)
{
	return (x < y) & (x > z);
}

K_TEMPLATE_SPEC(kUInt) K_INLINE bool kIsExcl(kUInt x, kUInt y, kUInt z)
{
	return (x < y) & (x > z);
}
...
#endif // FBXFILESDK_COMPONENTS_KBASELIB_KLIB_ITYPE_H
