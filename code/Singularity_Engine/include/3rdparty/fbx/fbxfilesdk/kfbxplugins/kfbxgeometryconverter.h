...
public:
    ...
    */
    ...
    /** Convert shape(s) and link(s) from source to destination geometry.
    * \param pNode        Pointer to the node containing the geometry.
    * \param pSrcGeom     Pointer to the source geometry.
...
public:
    ...
    *                   Now, the new mesh will have Normals, UVs, vertex color, material, textures
    */
    ...
    */
    ...
    /** Triangulate a mesh(support non-convex)
    * \param pMesh     Pointer to the mesh to triangulate.
    * \param pStatus   The status is \cture if the triangulation is successful for the whole mesh.
    * \return          Pointer to the new triangulated mesh.
    * \remarks         This method creates a new mesh, leaving the source mesh unchanged.
    */
    KFbxMesh* TriangulateMesh2(KFbxMesh const* pMesh, bool& pStatus);
...
protected:
    ...
    void UpdatePolygon(KFbxMesh *pNewMesh, KFbxMesh const *pRefMesh, int pPolygonIndex, int* pNewIndex, int &pVerticeIndexMeshTriangulated, int &pPolygonIndexMeshTriangulated, bool pUseOriginal);
